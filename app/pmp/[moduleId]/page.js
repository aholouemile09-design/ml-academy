...
export default function PmpModulePage() {
  ...
  const mod = getPmpModule(moduleId);
  const progress = useProgress();
  const [activeLesson, setActiveLesson] = useState(0);
  // "lesson" → une leçon | "final" → exercice global | "quiz" → QCM ou examen blanc
  ...
  if (!mod) {
    return (
      <div className="max-w-4xl mx-auto px-6 py-20 text-center">
        ...
        <Link href="/pmp" className="btn-primary mt-6">
          Retour au parcours PMP
        </Link>
      </div>
    );
  }
  ...
  const quiz = progress?.quizScores?.[`pmp-${mod.id}`];
  const isMock = mod.id === "examen-blanc";
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      <Link href="/pmp" className="text-sm text-slate-500 hover:text-accent-light">
        ← Parcours PMP
      </Link>
      ...
      <div className="flex items-center gap-4 mt-4 mb-8 flex-wrap">
        ...
        <div>
          ...
          <span className={`inline-block mt-1 px-2 py-0.5 rounded-full border text-xs font-medium ${PMP_LEVELS[mod.level].badge} ${PMP_LEVELS[mod.level].color}`}>
            {PMP_LEVELS[mod.level].label}
          </span>
        </div>
      </div>
      ...
      {/* Syllabus : le problem set n'est masqué que si le module a un exercice
          final, qui le reprend en fin de parcours. */}
      <ModuleSyllabus moduleId={mod.id} showProblemSet={!mod.finalExercise} />
        ...
        <aside className="space-y-2">
          ...
          {mod.quiz?.length > 0 && (
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-xl border text-sm font-semibold transition-colors ${
                view === "quiz" ? "border-accent bg-accent/10 text-white" : "border-ink-700 text-slate-300 hover:border-accent/50"
              }`}
            >
              {isMock ? "📝 Série d'échauffement" : "📝 Quiz de validation"}
              ...
              <div className="mt-8 pt-6 border-t border-ink-700 flex items-center justify-between flex-wrap gap-3">
                ...
                {activeLesson + 1 < mod.lessons.length ? (
                  ...
                  <button onClick={() => setView("quiz")} className="btn-secondary">
                    {isMock ? "Passer à l'examen blanc →" : "Passer au quiz →"}
                  </button>
                ) : null}
              </div>
            </article>
