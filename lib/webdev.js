...
import { WEB_EXERCISES } from "./exercises-web.js";
...
Tailwind est utilisé dans CE site (CodeGraft Academy) — tu peux inspecter les classes dans le code pour apprendre.
...
\`\`\``,
  ...
  {
    id: "typescript",
    title: "TypeScript : JavaScript avec des super-pouvoirs",
    level: "intermediaire",
    icon: "🔷",
    description:
      "TypeScript est le standard de l'industrie en 2025. Il ajoute des types statiques à JavaScript, détecte les bugs avant l'exécution et rend le code auto-documenté. Next.js, Prisma et Express l'utilisent nativement.",
    lessons: [
      {
        id: "ts-1",
        title: "Types, interfaces et annotations",
        duration: "55 min",
        resources: [
          { type: "doc", title: "TypeScript Handbook officiel", url: "https://www.typescriptlang.org/docs/handbook/intro.html", description: "La documentation officielle TypeScript — claire, exhaustive, avec playground interactif." },
          { type: "exercise", title: "TypeScript Exercises", url: "https://typescript-exercises.github.io/", description: "18 exercices progressifs du niveau débutant à avancé — corriges interactifs dans le navigateur." },
          { type: "exercise", title: "Total TypeScript — Beginners Tutorial", url: "https://www.totaltypescript.com/tutorials/beginners-typescript", description: "Cours interactif gratuit de Matt Pocock, le meilleur formateur TypeScript actuellement." },
        ],
        content: `TypeScript = JavaScript + types statiques. Le compilateur détecte les erreurs avant même d'exécuter le code.
...
**Ressource** : TypeScript Handbook — https://www.typescriptlang.org/docs/handbook/intro.html`,
      ...
      {
        id: "ts-2",
        title: "Generics, utilitaires et types avancés",
        duration: "55 min",
        resources: [
          { type: "exercise", title: "Total TypeScript — Type Transformations", url: "https://www.totaltypescript.com/workshops/type-transformations", description: "Workshop de Matt Pocock spécialement sur les types utilitaires et les transformations de types." },
          { type: "exercise", title: "Type Challenges", url: "https://github.com/type-challenges/type-challenges", description: "Collection de challenges TypeScript classés par difficulté — de easy à extreme. Le LeetCode du typage." },
        ],
        content: `Les generics permettent d'écrire du code réutilisable qui fonctionne avec n'importe quel type — sans perdre la sécurité des types.

**Generics — fonctions réutilisables typées**
\`\`\`typescript
// Sans generics : on perd le type
function premier(tableau: any[]): any { return tableau[0]; }

// Avec generics : le type est préservé
function premier<T>(tableau: T[]): T {
  return tableau[0];
}

const premiereLeçon = premier(["py-1", "py-2"]);  // string
const premierScore = premier([95, 87, 72]);         // number
\`\`\`

**Generics avec contraintes**
\`\`\`typescript
interface AvecId { id: string; }

// T doit avoir une propriété id
function trouverParId<T extends AvecId>(items: T[], id: string): T | undefined {
  return items.find(item => item.id === id);
}

// Fonctionne avec Module, Leçon, Utilisateur — n'importe quel objet avec id
const module = trouverParId(WEB_CURRICULUM, "typescript");
\`\`\`

**Types utilitaires intégrés** — les plus utiles en pratique :
\`\`\`typescript
interface Utilisateur {
  id: string;
  email: string;
...
type MiseÀJourUtilisateur = Partial<Utilisateur>;
// { id?: string; email?: string; nom?: string; ... }

// Required — toutes deviennent obligatoires
type UtilisateurComplet = Required<Utilisateur>;

// Pick — sous-ensemble de propriétés
type ProfilPublic = Pick<Utilisateur, "id" | "nom" | "avatarUrl">;

// Omit — exclure des propriétés
type SansMotDePasse = Omit<Utilisateur, "email">;

// Readonly — propriétés en lecture seule
type UtilisateurFigé = Readonly<Utilisateur>;

// Record — dictionnaire typé
type ScoresQuiz = Record<string, { score: number; total: number }>;
// équivalent à : { [moduleId: string]: { score: number; total: number } }
\`\`\`

**Types conditionnels et inférence avancée**
\`\`\`typescript
// Extraire le type de retour d'une fonction
type Retour<T> = T extends (...args: any[]) => infer R ? R : never;

async function fetchUser(): Promise<Utilisateur> { /* ... */ }
type TypeUser = Awaited<ReturnType<typeof fetchUser>>;  // Utilisateur

// Discriminated unions — pattern essentiel pour les états
type ÉtatRequête =
  | { status: "idle" }
  | { status: "loading" }
...
function gérerÉtat(état: ÉtatRequête) {
  switch (état.status) {
    case "success": return état.data;    // TypeScript sait que data existe ici
    case "error": return état.message;   // TypeScript sait que message existe ici
  }
}
\`\`\`

**Enums — constantes nommées**
\`\`\`typescript
enum Parcours { ML = "ml", Web = "web", PMP = "pmp" }

const parcoursCourant: Parcours = Parcours.ML;
// Préférer les string unions : type Parcours = "ml" | "web" | "pmp"
// Plus léger, pas compilé en objet JavaScript
\`\`\``,
      },
      {
        id: "ts-3",
        title: "TypeScript avec React et Next.js",
        duration: "60 min",
        content: `Next.js génère des projets TypeScript par défaut depuis la version 13. Voici comment typer correctement les composants React.
...
interface ModuleCardProps {
  ...
  onComplete?: (id: string) => void;  // fonction avec argument
}

export function ModuleCard({
  id, icon, title, level, lessonsCount,
  isCompleted = false,
  onClick,
  onComplete,
}: ModuleCardProps) {
  return (
    <div onClick={onClick} className="card p-6">
      <span>{icon}</span>
      <h3>{title}</h3>
      {onComplete && (
...
function QuizPlayer() {
  ...
  const [réponseChoisie, setRéponseChoisie] = useState<number | null>(null);
  const inputRef = useRef<HTMLInputElement>(null);

  const focusInput = () => {
    inputRef.current?.focus();  // ?. car peut être null avant le montage
  };
...
interface ProgressContextType {
  ...
  saveQuiz: (moduleId: string, score: number, total: number) => void;
}
...
export function useProgress(): ProgressContextType {
  ...
  return ctx;  // TypeScript sait que ctx n'est plus null après ce check
}
\`\`\`

**Typer les Server et Client Components Next.js**
\`\`\`tsx
// app/parcours/[moduleId]/page.tsx — Server Component
interface PageProps {
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const module = getModule(params.moduleId);
  return { title: module?.title ?? "Module introuvable" };
}
\`\`\`

**Typer les Route Handlers API**
\`\`\`typescript
...
export async function POST(req: NextRequest) {
  ...
  if (!body.lessonId || typeof body.xpGained !== "number") {
    return NextResponse.json({ error: "Invalid body" }, { status: 400 });
  }

  // body est maintenant correctement typé
  return NextResponse.json({ success: true, lessonId: body.lessonId });
}
\`\`\`

**Ressource** : React TypeScript Cheatsheet — https://react-typescript-cheatsheet.netlify.app`,
      },
      {
        id: "ts-4",
        title: "TypeScript avec Node.js, Express et Zod",
        duration: "55 min",
        content: `TypeScript côté backend + Zod pour la validation des données = une stack robuste et type-safe de bout en bout.

**Configurer TypeScript pour Node.js**
\`\`\`bash
...
// Typer le body d'une requête
interface PrédictionRequest {
  features: number[];
  modelVersion?: string;
}

interface PrédictionResponse {
  prediction: number;
  confidence: number;
  modelVersion: string;
}
...
app.post("/api/predict", async (
  req: Request<{}, PrédictionResponse, PrédictionRequest>,
  res: Response<PrédictionResponse>
) => {
  const { features, modelVersion = "v1" } = req.body;
  // TypeScript connaît le type de features : number[]
  const result = await runModel(features);
  res.json({ prediction: result.value, confidence: result.conf, modelVersion });
});
\`\`\`

**Zod — validation + inférence de type en une seule déclaration**
Zod valide les données d'entrée ET génère automatiquement les types TypeScript.

\`\`\`typescript
import { z } from "zod";

// Définir le schéma une seule fois
const PrédictionSchema = z.object({
  features: z.array(z.number()).min(1, "Au moins une feature requise"),
  modelVersion: z.enum(["v1", "v2"]).default("v2"),
  userId: z.string().uuid().optional(),
});

// TypeScript infère automatiquement le type depuis le schéma
type PrédictionInput = z.infer<typeof PrédictionSchema>;
// { features: number[]; modelVersion: "v1" | "v2"; userId?: string }

app.post("/api/predict", async (req: Request, res: Response) => {
  const résultat = PrédictionSchema.safeParse(req.body);

  if (!résultat.success) {
    return res.status(400).json({
      error: "Données invalides",
      details: résultat.error.flatten()
    });
  }

  // résultat.data est parfaitement typé ici
  const { features, modelVersion } = résultat.data;
  const prédiction = await runModel(features, modelVersion);
  res.json(prédiction);
});
\`\`\`

**Pattern repository typé — architecture propre**
\`\`\`typescript
...
\`\`\``,
    ...
    quiz: [
      {
        q: "Quelle est la différence principale entre `interface` et `type` en TypeScript ?",
        options: [
          "interface est plus rapide à compiler",
          "type peut définir des unions et intersections ; interface supporte l'extension avec extends et la déclaration multiple",
          "type ne fonctionne qu'avec les primitifs",
          "Il n'y a pas de différence pratique",
        ],
        answer: 1,
        explain: "Les deux se ressemblent pour les objets. Préférer interface pour les objets extensibles, type pour les unions/intersections et les alias de types complexes.",
      },
      {
        q: "Que fait le type utilitaire `Partial<T>` ?",
        options: [
          "Supprime la moitié des propriétés",
          "Rend toutes les propriétés de T optionnelles",
          "Rend toutes les propriétés de T en lecture seule",
          "Extrait les propriétés de type string",
        ],
        answer: 1,
        explain: "Partial<T> est parfait pour les fonctions de mise à jour où on ne veut envoyer que les champs modifiés.",
      },
      {
        q: "Pourquoi utiliser Zod plutôt qu'une simple interface TypeScript pour valider une requête API ?",
        options: [
          "Les interfaces TypeScript ne compilent pas en production",
          "TypeScript efface les types à l'exécution — Zod valide les données réelles au runtime ET génère les types",
          "Zod est plus rapide que TypeScript",
          "Zod remplace TypeScript",
        ],
        answer: 1,
        explain: "TypeScript disparaît après compilation. Zod tourne au runtime et rejette les données malformées avant qu'elles n'atteignent ta logique métier.",
      },
      {
        q: "Qu'est-ce qu'un generic `<T>` en TypeScript ?",
        options: [
          "Un type qui accepte uniquement des objets",
          "Un paramètre de type qui rend une fonction réutilisable tout en préservant la sécurité des types",
          "Un raccourci pour `any`",
          "Un type spécial pour les tableaux",
        ],
        answer: 1,
        explain: "Les generics permettent d'écrire du code générique qui fonctionne avec n'importe quel type sans sacrifier la vérification statique.",
      },
    ],
  },
  {
    ...
    icon: "⚛️",
    description:
      "Le framework standard pour les applications web modernes. Ce site CodeGraft Academy est construit avec Next.js — apprends en lisant son code.",
    lessons: [
      {
        id: "react-1",
        ...
        duration: "65 min",
        resources: [
          { type: "doc", title: "Documentation officielle React", url: "https://react.dev/learn", description: "La nouvelle doc React avec le paradigme modern (hooks, Server Components) — interactive et excellente." },
          { type: "course", title: "The Odin Project — React", url: "https://www.theodinproject.com/paths/full-stack-javascript/courses/react", description: "Parcours React complet et gratuit avec projets — l'une des meilleures ressources autodidactes en ligne." },
          { type: "doc", title: "Documentation officielle Next.js", url: "https://nextjs.org/docs", description: "App Router, Server Components, routing, data fetching — la référence pour ce site." },
        ],
...
**Ressource** : react.dev — https://react.dev (tutoriel officiel interactif)`,
      },
      {
        ...
        duration: "65 min",
        content: `Next.js est le framework React de référence. Il ajoute routing basé sur les fichiers, rendu serveur, optimisations automatiques — et c'est ce qui fait tourner CodeGraft Academy.
...
- Tester avec React Testing Library`,
  ...
  },
  {
    ...
    icon: "🗃️",
    description:
      "PostgreSQL avec Prisma ORM, MongoDB pour les données flexibles. Concevoir et interroger des bases pour des applications full stack. À faire AVANT le backend : ton API et ton authentification stockeront leurs données ici.",
    lessons: [
      {
        id: "db-1",
...
\`\`\``,
  ...
  },
  {
    ...
    icon: "🖥️",
    description:
      "Construire des serveurs, des APIs REST et gérer l'authentification avec Node.js et Express. S'appuie sur le module Bases de données : tes endpoints y liront et écriront leurs données.",
    lessons: [
      {
        id: "node-1",
...
// Même mécanique que pour le parcours ML : les exercices vivent dans
// lib/exercises-web.js et sont greffés ici une fois, au chargement, pour que
// tous les consommateurs de WEB_CURRICULUM voient les mêmes données.
for (const mod of WEB_CURRICULUM) {
  const extra = WEB_EXERCISES[mod.id];
  if (!extra) continue;
  for (const lesson of mod.lessons) {
    const exercises = extra.lessons?.[lesson.id];
    if (exercises) lesson.exercises = exercises;
  }
  if (extra.finalExercise) mod.finalExercise = extra.finalExercise;
  if (extra.quizExtra?.length) mod.quiz = [...mod.quiz, ...extra.quizExtra];
}
