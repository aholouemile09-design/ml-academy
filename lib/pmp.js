...
import { PMP_EXERCISES } from "./exercises-pmp.js";
...
**Types de questions** : choix unique (la majorité), **choix multiples** (« sélectionnez les 3 »), **appariement** (matching), **points chauds** (hotspot, cliquer sur un schéma), listes déroulantes et quelques **textes à trous**.

> 🆕 **Les deux nouveautés de 2026, à ne pas découvrir le jour J** :
> → **Les études de cas** (*case sets*) : un scénario long de plusieurs paragraphes suivi de **3 à 5 questions liées**. Il faut lire le cas une fois, bien, puis répondre aux questions sans y revenir en permanence.
> → **Les questions visuelles** : un tableau de bord, un burndown, un radiateur d'information ou un diagramme à interpréter avant de répondre.

**Répartition des approches** : environ **60 % agile et hybride**, 40 % prédictif. C'est un renversement par rapport à l'ancien examen — ne construis pas ta préparation sur du prédictif.
...
**Approches mélangées** : environ **60 % des questions portent sur l'agile ou l'hybride**, 40 % sur le prédictif (cycle en V). C'est un renversement par rapport à l'ancien examen, où l'équilibre était de moitié-moitié. Ces approches sont réparties dans les 3 domaines, jamais isolées.
...
// Même mécanique que pour les parcours ML et Web : les exercices vivent dans
// lib/exercises-pmp.js et sont greffés ici une fois, au chargement.
for (const mod of PMP_CURRICULUM) {
  const extra = PMP_EXERCISES[mod.id];
  if (!extra) continue;
