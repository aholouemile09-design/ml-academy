...
import { todayStr, computeStreak } from "@/lib/streak";
...
function rowFromDb(row) {
  return {
    ...
    weeklyGoalType: row.weekly_goal_type || "lessons",
    weeklyGoalTarget: row.weekly_goal_target || 5,
    reminderOptIn: row.reminder_opt_in || false,
    studyPlan: row.study_plan || null,
    lessonNotes: row.lesson_notes || {},
    moduleTime: row.module_time || {},
  };
}

function recordActivity(currentDates, currentRecord) {
  const today = todayStr();
  const dates = currentDates.includes(today) ? currentDates : [...currentDates, today];
  const { current } = computeStreak(dates);
  const record = Math.max(currentRecord, current);
  return { activityDates: dates, streakRecord: record };
}
...
export function UserProgressProvider({ children }) {
  ...
  const persist = async (patch) => {
    ...
    if ("activityDates" in patch) dbPatch.activity_dates = patch.activityDates;
    if ("streakRecord" in patch) dbPatch.streak_record = patch.streakRecord;
    if ("weeklyGoalType" in patch) dbPatch.weekly_goal_type = patch.weeklyGoalType;
    if ("weeklyGoalTarget" in patch) dbPatch.weekly_goal_target = patch.weeklyGoalTarget;
    if ("reminderOptIn" in patch) dbPatch.reminder_opt_in = patch.reminderOptIn;
    if ("studyPlan" in patch) dbPatch.study_plan = patch.studyPlan;
    if ("avatarUrl" in patch) dbPatch.avatar_url = patch.avatarUrl;
    if ("lessonNotes" in patch) dbPatch.lesson_notes = patch.lessonNotes;
    if ("moduleTime" in patch) dbPatch.module_time = patch.moduleTime;
  ...
  const completeLesson = (lessonId) => {
    ...
    const activity = recordActivity(data.activityDates, data.streakRecord);
    persist({ completedLessons: [...data.completedLessons, lessonId], xp: data.xp + 50, ...activity });
  };
  ...
  const saveLessonNote = (lessonId, text) => {
    persist({ lessonNotes: { ...data.lessonNotes, [lessonId]: text } });
  };

  /** Enregistre le temps passé sur un module (on garde le maximum : le
   *  compteur local peut être en retard sur une autre machine). */
  const saveModuleTime = (moduleId, seconds) => {
    if (!moduleId || !seconds) return;
    const current = data.moduleTime?.[moduleId] || 0;
    if (seconds <= current) return;
    persist({ moduleTime: { ...data.moduleTime, [moduleId]: Math.round(seconds) } });
  };

  const updateGoal = (goalType, goalTarget, reminderOptIn) => {
    persist({ weeklyGoalType: goalType, weeklyGoalTarget: goalTarget, reminderOptIn });
  };

  const resetProgress = () => persist({ completedLessons: [], quizScores: {}, calendarChecks: {}, xp: 0, activityDates: [], streakRecord: 0 });
  ...
  return (
    <UserProgressContext.Provider
      value={{
        ...
        updateProfile,
        updateGoal,
        saveStudyPlan: (plan) => persist({ studyPlan: plan }),
        saveLessonNote,
        saveModuleTime,
        signOut,
        importLegacy,
