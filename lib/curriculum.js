...
> 💡 **Choisis ton environnement ci-dessous** — les commandes sont regroupées par shell pour que tu puisses copier directement ce qui te correspond.

---

## 🐧 bash / zsh — Mac et Linux
...
> PowerShell est bien plus puissant que CMD. C'est le terminal par défaut dans VS Code et PyCharm sur Windows.

**Navigation :**
\`\`\`powershell
pwd                      # dossier courant (alias de Get-Location)
ls                       # liste (alias de Get-ChildItem)
cd projet                # aller dans un dossier
cd ..                    # remonter d'un niveau
mkdir src                # créer un dossier
cp a.py b.py             # copier un fichier
...
rm -Recurse dossier      # supprimer un dossier entier
\`\`\`

**Fichiers de données :**
\`\`\`powershell
cat data.csv                          # afficher le contenu
Get-Content data.csv -TotalCount 20   # 20 premières lignes
(Get-Content data.csv).Count          # compter les lignes
Select-String "erreur" logs.txt       # chercher dans un fichier
\`\`\`
...
> CMD est limité. Si tu es bloqué sur CMD, préfère installer PowerShell ou Git Bash.

**Navigation :**
\`\`\`cmd
cd                       # dossier courant (sans argument)
dir                      # liste des fichiers
cd projet                # aller dans un dossier
cd ..                    # remonter d'un niveau
mkdir src                # créer un dossier
copy a.py b.py           # copier un fichier
move a.py src            # déplacer / renommer
del fichier.py           # supprimer un fichier
rmdir /s dossier         # supprimer un dossier entier
\`\`\`

**Fichiers de données :**
\`\`\`cmd
type data.csv            # afficher le contenu
findstr "erreur" logs.txt  # chercher dans un fichier
\`\`\`
...
> ⚠️ Ces commandes définissent la variable **pour la session courante uniquement**. Pour la rendre permanente dans un projet, utilise un fichier \`.env\` avec la librairie python-dotenv — **ne mets jamais tes clés dans ton code source !**
